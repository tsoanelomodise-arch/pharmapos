...
export const generateHelpPdf = () => {
  ...
  addListItem('Product Name: The primary display name for this product');
  addListItem('Generic Name: The active ingredient or generic pharmaceutical name');
  addListItem('Brand: The manufacturer or brand name of this product');
  addListItem('Category: Product classification (Prescription, OTC, Supplement, Medical Device, Cosmetic)');
  addListItem('Barcode: Unique barcode identifier for scanning');
  addListItem('Description: Additional details, ingredients, or usage instructions');
  addListItem('Unit Price: Selling price per unit to customers');
  addListItem('Cost Price: Your purchase cost per unit from supplier');
  addListItem('Stock Quantity: Current number of units in inventory');
  addListItem('Minimum Stock: Alert threshold - system warns when stock falls below this level');
  addListItem('Expiry Date: Product expiration date for tracking shelf life');
  addListItem('Batch Number: Manufacturer batch/lot number for traceability');
  addListItem('Requires Prescription: Check if this medication requires a valid prescription');
  addSpacer(5);
