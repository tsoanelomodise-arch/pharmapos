...
export const generateHelpPdf = () => {
  ...
  addListItem('Product Name: Name on packaging (e.g., Panado 500mg Tablets)');
  addListItem('Generic Name: Active ingredient (e.g., Paracetamol)');
  addListItem('Brand: Manufacturer (e.g., Adcock Ingram)');
  addListItem('Category: Product type - Prescription, OTC, Supplement, Medical Device, Cosmetic');
  addListItem('Barcode: For POS scanning (e.g., 6001505012345)');
  addListItem('Description: Extra details (e.g., 24 tablets per pack, sugar-free)');
  addListItem('Unit Price: Selling price (e.g., R45.99)');
  addListItem('Cost Price: Your purchase cost (e.g., R32.50)');
  addListItem('Stock Quantity: Units in stock (e.g., 150)');
  addListItem('Minimum Stock: Low stock alert threshold (e.g., 20)');
  addListItem('Expiry Date: When product expires (e.g., 2025-12-31)');
  addListItem('Batch Number: Lot number for tracking (e.g., BN2024-0815)');
  addListItem('Requires Prescription: ON for Schedule 3-6 medicines');
  addSpacer(5);
