...
## Goal
Replace the Previous/Next pagination on Transactions > Transaction History with a **Load more** button at the bottom of the list. Each click shows 80 additional transactions **without removing** any already displayed.
...
4. Add a footer under the table:
   ...
   - **Load more** button (outline style) visible only when `visibleCount < sales.length`; on click: `setVisibleCount(c => c + 80)`.
...
- Behaviour: first load shows 20 rows → each Load more click adds 80 → button disappears once all matching transactions are shown.
